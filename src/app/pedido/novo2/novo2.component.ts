...
export class Novo2Component implements OnInit {
  ...
  async volk(data) {
    ...
    var final = 0;
    var clienteCnpj =
      data[7][3].toString().length == 13 ? "0" + data[7][3] : data[7][3];
    var pedido =
      data[21][12] != undefined ? data[6][1] + "/" + data[21][12] : data[6][1];
    ...
    this.form
      .get("data_emissao")
      .setValue(
        moment(data[6][3].replace(/\//g, "-"), "DD-MM-YYYY").format(
          "YYYY-MM-DD"
        )
      );
    this.form
      .get("data_entrega")
      .setValue(moment(data[21][4], "DD-MM-YYYY").format("YYYY-MM-DD"));
    this.form
      .get("frete")
      .setValue(data[17][1] == "C" ? "Cliente" : "Representada");
  ...
  async camper(data) {
    ...
    let clienteCnpj = data[2][0]
      .toString()
      .match(new RegExp("\\d{2}\\.\\d{3}\\.\\d{3}\\/\\d{4}\\-\\d{2}", "g"))[0];
    clienteCnpj = clienteCnpj.replace(/[^\d]+/g, "");
    clienteCnpj = clienteCnpj.length == 13 ? "0" + clienteCnpj : clienteCnpj;
    this.form
      .get("num_pedido")
      .setValue(data[0][0].toString().match(new RegExp("\\d+", "g"))[0]);
  ...
  async kadesh(data) {
    ...
    var final = 0;
    this.condComercial =
      data[12][0].split(" ")[0] + " " + data[12][0].split(" ")[1];

    let clienteCnpj = data[7][0]
      .toString()
      .match(new RegExp("\\d{14}", "g"))[0];
    ...
    this.form
      ...
      .setValue(
        moment(
          data[4][0]
            .match(new RegExp("\\d{2}\\/\\d{2}\\/\\d{4}", "g"))[0]
            .replace(/\//g, "-"),
          "DD-MM-YYYY"
        ).format("YYYY-MM-DD")
      );
    ...
    this.form
      .get("frete")
      .setValue(
        data[final + 9][0] == "CIF Destino" ? "Cliente" : "Representada"
      );
  ...
  async betanin(data) {
    ...
    this.form
      .get("data_emissao")
      .setValue(
        moment(data[3][31].replace(/\//g, "-"), "DD-MM-YYYY").format(
          "YYYY-MM-DD"
        )
      );
    ...
    this.form
      .get("frete")
      .setValue(data[final + 8][5] == "CIF" ? "Cliente" : "Representada");
  ...
  async italbotas(data) {
    ...
    this.form
      .get("num_pedido")
      .setValue(data[5][1].trim() + "/" + data[6][27].trim());
    this.form
      .get("data_emissao")
      .setValue(
        moment(
  ...
  valorTotal(tipo) {
    ...
    if (tipo == "total") return total + ipi - desconto;
