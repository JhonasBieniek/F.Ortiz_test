...
@Component({
  selector: 'app-dialog-devolucoes-print',
  templateUrl: './dialog-devolucoes-print.component.html',
  styleUrls: ['./dialog-devolucoes-print.component.css'],
  providers:[
    DatePipe,MaskPipe
  ]
})
export class DialogDevolucoesPrintComponent implements OnInit {
  ...
  export_to_excell(){
    ...
    let aoaArray: any[] = [
      ...
      [ null],
      ['Representada', 'Cliente', "Núm. Nota", 'Núm. Pedido', 'Quantidade', 'Total'],
    ];
    ...
    this.data.registros.forEach((row:any) => {
        ...
        row.nota_produto_devolutions.forEach((produto:any) => {
          let new_produto = [
            null,
            produto.pedido_produto.produto.codigo_catalogo,
            produto.pedido_produto.produto.nome,
            produto.pedido_produto.valor_unitario,
            produto.qtd,
            produto.qtd * produto.pedido_produto.valor_unitario,
          ];

          aoaArray.push(new_produto);
        });
    ...
    /* save to file */
    XLSX.writeFile(wb,'Relatorio de Devolucoes.xlsx');
  }
